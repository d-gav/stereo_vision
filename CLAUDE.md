...
- [computer_15_640_video_mod/computer_15_640_video_mod/](computer_15_640_video_mod/computer_15_640_video_mod/) — Quartus project (forked from the U.Toronto "Computer System 15" 640x480 video-mod baseline). Top-level is [DE1_SoC_Computer.v](computer_15_640_video_mod/computer_15_640_video_mod/verilog/DE1_SoC_Computer.v); Qsys subsystems are `Computer_System.qsys`, `VGA_Subsystem.qsys`, `Video_In_Subsystem.qsys`, plus the newer `Edge_Detection_Subsystem.qsys`. Stereo RTL lives alongside in `verilog/` (`block_match.sv` + `block_match_pkg.sv`, `mem_block_intf*.v`, `sliding_window.sv`, `column_prefetch.v` and the parallel-row variant `column_prefetch_parallel.sv`, `stereo_bram_bank.v`, the custom `stereo_onchip_ram.sv` that replaces the Qsys `Onchip_SRAM` IP, plus the radial/polynomial undistortion mappers — `stereo_radial_mapper_q15*.v` (incl. `_pipe`/`_pipe_compat` pipelined variants), `stereo_poly_mapper_deg6_q18.v`, `rectification_map.sv`, `undistort_ring.sv`).
...
- **Parallel row-read path.** The newer pixel-fetch path replaces the Qsys `altera_avalon_onchip_memory2` IP (`Onchip_SRAM` in `Computer_System.qsys`) with the custom [stereo_onchip_ram.sv](computer_15_640_video_mod/computer_15_640_video_mod/verilog/stereo_onchip_ram.sv). It is Avalon-drop-in compatible (same s1 8-bit and s2 32-bit slaves, same 200 KB / 200 rows × 1024-byte stride, 1-cycle read latency) but adds a side-door `sad_*` conduit that exposes port B of every row's M10K so the SAD engine reads one byte from EVERY row in parallel each cycle. [column_prefetch_parallel.sv](computer_15_640_video_mod/computer_15_640_video_mod/verilog/column_prefetch_parallel.sv) is the drop-in replacement for `column_prefetch.v` that consumes that conduit; the external interface to `mem_block_intf` (req/bank/col + per-row `mem_rdata` array + `stall`) is unchanged. Capture / SAD compute / VGA-readback are workflow-separated phases, so the shared M10K port-B priority (s1 > s2 > SAD) is safe in normal operation. If you regenerate Qsys you must re-wire `Onchip_SRAM` to the custom RTL; do not let Qsys drop in the stock IP.
...
- **Active RTL is the most recently named sibling, not the bare-name file.** The same "newer suffixed variant lives next to an older snapshot" pattern shows up several places: `mem_block_intf_d.v` (active) vs `mem_block_intf.v` (snapshot); `column_prefetch_parallel.sv` (active, parallel-row-read) vs `column_prefetch.v` (older one-row-at-a-time); `stereo_radial_mapper_q15_pipe.v` / `_pipe_compat.v` (pipelined) vs `stereo_radial_mapper_q15.v`; and `block_match.sv` vs `block_match copy.sv`. Edit the suffixed/newer one and check the top-level instantiation in [DE1_SoC_Computer.v](computer_15_640_video_mod/computer_15_640_video_mod/verilog/DE1_SoC_Computer.v) before assuming which is wired in.
