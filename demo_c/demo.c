...
// SGM penalty PIOs: 32-bit HPS -> FPGA outputs on the LW AXI master.
// pio_small_pen lives at LW offset 0x10, pio_big_pen at LW offset 0x20.
// They drive the +/-1 and >+/-1 disparity smoothness penalties in mem_block_intf.
#define PIO_SMALL_PEN_OFFSET 0x10u
#define PIO_BIG_PEN_OFFSET   0x20u
volatile unsigned int *h2p_lw_pio_small_pen=NULL;
volatile unsigned int *h2p_lw_pio_big_pen=NULL;

// Defaults match the previous mem_block_intf parameter values.
#define DEFAULT_SMALL_PEN 8u
#define DEFAULT_BIG_PEN   32u
...
static void print_usage(const char *prog)
{
	fprintf(stderr,
		"Usage: %s [options]\n"
		"  --small-pen N        SGM +/-1 disparity penalty (pio_small_pen, 32-bit). Default %u.\n"
		"  --big-pen   N        SGM >+/-1 disparity penalty (pio_big_pen,   32-bit). Default %u.\n"
		"  -s N                 alias for --small-pen\n"
		"  -b N                 alias for --big-pen\n"
		"  -h, --help           show this help\n"
		"\n"
		"N may be decimal (123), hex (0x7B) or octal (0173). The value is written to the\n"
		"corresponding PIO at startup; the FPGA latches it for the next stereo run.\n",
		prog, DEFAULT_SMALL_PEN, DEFAULT_BIG_PEN);
}

static int parse_uint_arg(const char *arg, unsigned int *out)
{
	char *end = NULL;
	unsigned long v;
	...
	errno = 0;
	v = strtoul(arg, &end, 0);
	if (errno != 0 || end == arg || *end != '\0' || v > 0xFFFFFFFFul) {
		return -1;
	}
	*out = (unsigned int)v;
	return 0;
...
int main(int argc, char *argv[])
{
	...
	unsigned int small_pen = DEFAULT_SMALL_PEN;
	unsigned int big_pen   = DEFAULT_BIG_PEN;
	int i;
	...
	for (i = 1; i < argc; i++) {
		...
		if (strcmp(a, "--small-pen") == 0 || strcmp(a, "-s") == 0) {
			if (i + 1 >= argc) {
			...
			val = argv[++i];
			if (parse_uint_arg(val, &small_pen) != 0) {
		...
		if (strcmp(a, "--big-pen") == 0 || strcmp(a, "-b") == 0) {
			if (i + 1 >= argc) {
				fprintf(stderr, "ERROR: %s requires a value\n", a);
				print_usage(argv[0]);
				return 1;
			...
			val = argv[++i];
			if (parse_uint_arg(val, &big_pen) != 0) {
				fprintf(stderr, "ERROR: bad value for %s: '%s'\n", a, val);
				return 1;
			}
			continue;
	...
	printf("SGM penalties: small_pen=%u (0x%08X)  big_pen=%u (0x%08X)\n",
		small_pen, small_pen, big_pen, big_pen);
	...
	// SGM penalty PIOs (LW base offsets, see Computer_System.qsys).
	h2p_lw_pio_small_pen=(volatile unsigned int *)((char *)h2p_lw_virtual_base + PIO_SMALL_PEN_OFFSET);
	h2p_lw_pio_big_pen  =(volatile unsigned int *)((char *)h2p_lw_virtual_base + PIO_BIG_PEN_OFFSET);
	*h2p_lw_pio_small_pen = small_pen;
	*h2p_lw_pio_big_pen   = big_pen;
	printf("Wrote pio_small_pen <- %u, pio_big_pen <- %u\n", small_pen, big_pen);
