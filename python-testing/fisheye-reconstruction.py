...
crops it, then applies a source-lens -> pinhole remap so the result
...
class CameraProfile:
	...
	default_projection_model: str
	default_center_x_offset_px: float = 0.0
	default_center_y_offset_px: float = 0.0
	default_residual_k1: float = 0.0
	default_residual_k2: float = 0.0
...
CAMERA_PROFILES: dict[str, CameraProfile] = {
	...
	"razer_v2": CameraProfile(
		...
		default_correction_strength=1.0,
		default_projection_model="rectilinear",
		default_center_x_offset_px=0.0,
		default_center_y_offset_px=0.0,
		default_residual_k1=0.0,
		default_residual_k2=0.0,
	),
...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Find 320x240 active region in NTSC captures and rectify it."
	)
	parser.add_argument(
		"--input-dir",
		type=Path,
	...
	parser.add_argument(
		...
		help="Camera profile to use for lens remapping.",
	)
	parser.add_argument(
		"--input-hfov-deg",
		type=float,
	...
	parser.add_argument(
		...
		default=None,
		help=(
			"Blend factor in [0,2] between identity remap (0) and full model (1). "
			"Values >1 intentionally over-correct. If omitted, uses profile default."
		),
	)
	parser.add_argument(
		"--projection-model",
		type=str,
		default=None,
		choices=PROJECTION_MODELS,
		help=(
			"Projection model used by the source lens. "
	...
	parser.add_argument(
		...
		default=None,
		help=(
			"Source lens center horizontal offset in pixels. "
			"Positive shifts center right; if omitted, uses profile default."
		),
	)
	parser.add_argument(
		"--center-y-offset-px",
		...
		default=None,
		help=(
			"Source lens center vertical offset in pixels. "
			"Positive shifts center down; if omitted, uses profile default."
		),
	)
	parser.add_argument(
		"--residual-k1",
		...
		default=None,
		help=(
			"Residual radial correction k1 term applied after model mapping. "
			"If omitted, uses profile default."
		),
	)
	parser.add_argument(
		...
		default=None,
		help=(
			"Residual radial correction k2 term applied after model mapping. "
			"If omitted, uses profile default."
		),
	)
	parser.add_argument(
...
def build_lens_to_pinhole_map(
	...
	center_x_offset_px: float = 0.0,
	center_y_offset_px: float = 0.0,
	residual_k1: float = 0.0,
	residual_k2: float = 0.0,
) -> tuple[np.ndarray, np.ndarray]:
	"""Create remap grids from a source lens projection to pinhole output."""
	...
	if not (0.0 <= correction_strength <= 2.0):
		raise ValueError("correction_strength must be in [0, 2]")
	if projection_model not in PROJECTION_MODELS:
		raise ValueError(
			f"projection_model must be one of {PROJECTION_MODELS}, got {projection_model!r}"
		)

	cx_src = (src_width - 1) * 0.5 + float(center_x_offset_px)
	cy_src = (src_height - 1) * 0.5 + float(center_y_offset_px)
	cx_out = (src_width - 1) * 0.5
	cy_out = (src_height - 1) * 0.5

	theta_max = input_hfov_rad * 0.5
	r_max = src_width * 0.5
	if projection_model == "equidistant":
		# r = f * theta
		f_model = r_max / theta_max
	elif projection_model == "equisolid":
		# r = 2 * f * sin(theta/2)
		f_model = r_max / (2.0 * np.sin(theta_max * 0.5))
	elif projection_model == "stereographic":
		# r = 2 * f * tan(theta/2)
		f_model = r_max / (2.0 * np.tan(theta_max * 0.5))
	elif projection_model == "orthographic":
		# r = f * sin(theta)
		f_model = r_max / np.sin(theta_max)
	else:
		# rectilinear (pinhole): r = f * tan(theta)
		f_model = r_max / np.tan(theta_max)
	...
	theta = np.arctan(rho)
	if projection_model == "equidistant":
		r_model = f_model * theta
	elif projection_model == "equisolid":
		r_model = 2.0 * f_model * np.sin(theta * 0.5)
	elif projection_model == "stereographic":
		r_model = 2.0 * f_model * np.tan(theta * 0.5)
	elif projection_model == "orthographic":
		r_model = f_model * np.sin(theta)
	else:
		r_model = f_model * np.tan(theta)
	...
	# Optional residual radial correction for fine plumb-line tuning.
	tan_half_out = np.tan(output_hfov_rad * 0.5)
	rho_norm = rho / max(1e-6, float(tan_half_out))
	rho2 = rho_norm * rho_norm
	rho4 = rho2 * rho2
	r_residual = 1.0 + residual_k1 * rho2 + residual_k2 * rho4
	r_src = r_src * r_residual
...
def rectify_crop(
	...
	map_x, map_y = build_lens_to_pinhole_map(
		src_width=w,
		src_height=h,
...
def main() -> None:
	...
	projection_model = args.projection_model or profile.default_projection_model
	center_x_offset_px = (
		args.center_x_offset_px
		if args.center_x_offset_px is not None
		else profile.default_center_x_offset_px
	)
	center_y_offset_px = (
		args.center_y_offset_px
		if args.center_y_offset_px is not None
		else profile.default_center_y_offset_px
	)
	residual_k1 = (
		args.residual_k1
		if args.residual_k1 is not None
		else profile.default_residual_k1
	)
	residual_k2 = (
		args.residual_k2
		if args.residual_k2 is not None
		else profile.default_residual_k2
	)
	...
	print(
		...
		f"projection={projection_model}, center_offset=({center_x_offset_px:.2f}, {center_y_offset_px:.2f}), "
		f"residual=(k1={residual_k1:.4f}, k2={residual_k2:.4f})"
	)
